...
export function RoomClient({ room, initialParticipants }: RoomClientProps) {
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 dark:bg-slate-950">
      ...
      {/* Circle Modal for Mobile */}
      {showCircleModal && (
        <div className="fixed inset-0 bg-black/50 z-50 lg:hidden" onClick={() => setShowCircleModal(false)}>
          <div className="absolute inset-x-4 top-20 bottom-20 bg-white dark:bg-slate-900 rounded-2xl p-6 flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
                {locale === "en" ? "Participants" : "شرکت کنندگان"}
              </h3>
              <Button variant="ghost" size="sm" onClick={() => setShowCircleModal(false)}>
                ✕
              </Button>
            </div>
      ...
      {/* Compact Header */}
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-3 py-2 flex-shrink-0">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <Button
              ...
              size="sm"
              onClick={() => router.push("/")}
              className="text-slate-600 dark:text-slate-400 px-2"
            >
        ...
        {/* Mode Switcher - Always visible when in room */}
        {isInRoom && (
      ...
      <div className="flex-1 overflow-hidden">
        {!isInRoom ? (
          ...
          <div className="h-full flex items-center justify-center p-4">
            <div className="max-w-md w-full text-center space-y-4">
              ...
              <Button
                ...
                size="lg"
                className="bg-teal-600 hover:bg-teal-700 text-white w-full py-6 text-base"
              >
                {isJoining
                  ? locale === "en"
                    ? "Joining..."
          ...
          /* In Room - Mobile: Chat First, Desktop: Split */
          <div className="h-full flex flex-col lg:flex-row">
            {/* Desktop: Circle on Left */}
            <div className="hidden lg:flex lg:w-[45%] border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex-col">
